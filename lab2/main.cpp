...
template<typename T, int P> class TBTree { // T - тип ключа, P - характеристика дерева
    template<typename, int> struct TBTreeNode { // узел B-ƒерева
        ...
        TBTreeNode() : N(0), Leaf(true) { // по умолчанию в узле ничего не хранится и узел является листом
            Keys = new T[2 * P - 1];
            Nodes = new TBTreeNode<T, P> * [2 * P];
            for (int i = 0; i < 2 * P; ++i)
                Nodes[i] = nullptr;
        }
        ~TBTreeNode() { // удаляются все ключи в узле и ссылки на другие, при этом дочерние остаются в памяти
            if (Keys) {
                delete[] Keys;
                Keys = nullptr;
            }
        ...
        void BTreeDeleteFromNonLeaf(int index) {//удаление ключа из узла, который является листом
            T k = Keys[index];
            if (Nodes[index]->N>= P) {
        ...
        T BTreeNodeGetPred(int index) { //достать у узла с индексом index - самый близкий слева узел хранящийся в дереве
        ...
        T BTreeNodeGetSucc(int index) {//достать у узла с индексом index - самый близкий справа узел хранящийся в дереве
